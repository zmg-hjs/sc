...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,tabsBodyChange: function(index, options){
      ...
      //修改滚动条样式
      var iframeDocument = admin.tabsBody(index).find('iframe')[0].contentDocument;
      var style = iframeDocument.createElement('style');
      style.type = 'text/css';
      style.id = 'scrollbarStyle';
      var scrollbarCss = '::-webkit-scrollbar-track,::-webkit-scrollbar-corner{background-color:#fff}::-webkit-scrollbar-thumb{border-radius:3px;background-color:rgba(97,183,207,1.3)}::-webkit-scrollbar-thumb:vertical:hover{background-color:rgba(97,183,207,1.35)}::-webkit-scrollbar-thumb:vertical:active{background-color:rgba(97,183,207,1.38)}';
      if($('#scrollbarStyle').length>0){
        $('#scrollbarStyle').html(scrollbarCss);
      }else{
        style.innerHTML = scrollbarCss;
        iframeDocument.getElementsByTagName('HEAD').item(0).appendChild(style);
      }
