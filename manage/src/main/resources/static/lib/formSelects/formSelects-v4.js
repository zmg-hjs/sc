...
(function (layui, window, factory) {
	...
	var v = 'static.4.0.0.0910',
	    ...
	    quickBtns = [{ icon: 'xm-iconfont icon-quanxuan', name: window.checkAllLang || '全选', click: function click(id, cm) {
			cm.selectAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-qingkong', name: window.emptyLang || '清空', click: function click(id, cm) {
			cm.removeAll(id, true, true);
		} }, { icon: 'xm-iconfont icon-fanxuan', name: window.selectInvertLang || '反选', click: function click(id, cm) {
	...
	Common.prototype.init = function (target) {
		...
		$(target ? target : 'select[' + NAME + ']').each(function (index, select) {
			...
			var heightStyle = !fs.config.height || fs.config.height == 'auto' ? '' : 'xm-hg style="height: 32px;"';
