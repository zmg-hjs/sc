...
layui.config({
    ...
    */
    ...
    function showOrderPicList() {
        var foodPicListStr = $("#orderPicStr").val();
        if(foodPicListStr != null && foodPicListStr.length > 10) {
            var foodPicList = JSON.parse(foodPicListStr);
            for (var i = 0; i < foodPicList.length; i++) {
                var id = getfileName(foodPicList[i]);
                $('#upload_img_list_orderPicStr').append('<dd class="item_img" id="' + id + '"><div class="operate"><i class="toleft1 layui-icon"></i><i class="toright1 layui-icon"></i><i onclick=UPLOAD_IMG_DEL("' + id + '") class="close layui-icon"></i></div><img src="' + foodPicList[i] + '" class="img" ><input type="hidden" name="dzd_img[]" value="' + foodPicList[i] + '" /></dd>');
            }
        }
    }
    ...
    function  getOrderPicStr() {
        var foodlist= $("#upload_img_list_orderPicStr").find("input");
        var list=new Array();
    ...
    layui.use('upload', function() {
        ...
        upload.render({
            ...
            before: function(obj) {
                ...
                layer.close(layer.msg());//关闭上传提示窗口
                if (duotu == true) {//调用多图上传方法,其中res.imgid为后台返回的一个随机数字
                    $('#upload_img_list_orderPicStr').append('<dd class="item_img" id="' + res.data + '"><div class="operate"><i class="toleft1 layui-icon"></i><i class="toright1 layui-icon"></i><i onclick=UPLOAD_IMG_DEL("' + res.data + '") class="close layui-icon"></i></div><img src="' + res.data + '" class="img" ><input type="hidden" name="dzd_img[]" value="' + res.data + '" /></dd>');
                }else{//调用单图上传方法,其中res.imgid为后台返回的一个随机数字
                    $('#upload_img_list_orderPicStr').append('<dd class="item_img" id="' + res.data + '"><div class="operate"><i onclick=UPLOAD_IMG_DEL("' + res.data + '") class="close layui-icon"></i></div><img src="' + res.data + '" class="img" ><input type="hidden" name="dzd_img" value="' + res.data + '" /></dd>');
                }
            }
        })
    });
    ...
    */
    ...
    $(document).on('click', '.toleft1', function () {
        var item = $(this).parent().parent(".item_img");
        var item_left = item.prev(".item_img");
        if ($("#upload_img_list").children(".item_img").length >= 2) {
            if (item_left.length == 0) {
    ...
    $(document).on('click', '.toright1', function () {
        var item = $(this).parent().parent(".item_img");
        var item_right = item.next(".item_img");
        if ($("#upload_img_list").children(".item_img").length >= 2) {
            if (item_right.length == 0) {
    ...
    layui.use('form', function () {
        ...
        form.on('submit(component-form)', function (data) {
            ...
            var orderPicStr= getOrderPicStr();
            searchObj.orderPicStr=orderPicStr;
            ...
            layer.open({
                ...
                ,yes:function(index, layero){
                    console.info("yes");
                    searchObj.serviceNoStr="view_push_update_submission";
                    $.simpleAjax('/view/v2/push/view_push_update_submission', 'POST', JSON.stringify(searchObj), "application/json;charset-UTF-8", returnFunction);
                    layer.close(index);
                }
                ,btn2:function (index) {
                    console.info("btn2");
