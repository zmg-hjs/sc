layui.config({
    ...
    table.render({
        ...
        , url: '/view/v2/business/view_business_index_data?serviceNoStr=view_business_index_data'
        , toolbar: '#order-table-toolbar-toolbarDemo'
        , title: ''
        , cols: [[
            {type: 'checkbox', field: 'id', fixed: 'left'}
            , {field: 'shopName', title: '店铺名', minWidth: 150,fixed: 'left', align: 'center'}
            , {field: 'fullName', title: '姓名', fixed: 'left', width: 150, align: 'center'}
            , {field: 'platformStr', title: '平台', width: 100, align: 'center'}
            /*, {field: 'brandStr', title: '品牌', minWidth: 100, align: 'center'}*/
            , {field: 'longitude', title: '经度', width: 150, align: 'center'}
            , {field: 'latitude', title: '维度', width: 150, align: 'center'}
            , {field: 'address', title: '店铺地址', minWidth: 200, align: 'center'}
            , {
                field: 'appletQrCodeUrl',
                title: '商家二维码',
                width: 150,
                align: 'center',
                templet: function(item) {
                    var appletQrCodeUrl = item.appletQrCodeUrl;
                    var result = '<a href="'+ appletQrCodeUrl + '" target="_blank " title="点击查看">' +
                        '<img src="' + appletQrCodeUrl + '" style="height:100%;" />' +
            ...
            , {
                ...
                align: 'center',
                templet: function(item) {
                    var logoAddress = item.logoAddress;
                    var result = '<a href="'+  logoAddress + '" target="_blank " title="点击查看">' +
                        '<img src="'+  logoAddress + '" style="height:100%;" />' +
                        '</a>'+'&nbsp&nbsp&nbsp&nbsp';
                    return result;
                }
            }
            , {
                ...
                align: 'center',
                templet: function(item) {
                    var brandAddress = item.brandAddress;
                    var result = '<a href="'+  brandAddress + '" target="_blank " title="点击查看">' +
                        '<img src="'+  brandAddress + '" style="height:100%;" />' +
                        '</a>'+'&nbsp&nbsp&nbsp&nbsp';
                    return result;
                }
            }
    ...
    table.on('tool(order-table-toolbar)', function (obj) {
        ...
        if (obj.event === 'update'){
            ...
            layer.open({
                ...
                area: [width, height],
                title: '商家修改页面',
                content: "/view/v2/business/view_business_update_page?id=" + data.id+"&serviceNoStr=view_business_detail_data"
                ,maxmin: true
