layui.config({
    ...
    function getImgSrc() {
        ...
        $("img[data-key^='filePreview_']").each(function (index, item) {
            var ss = $(item).attr("brand_url")
            if ($(item).attr("brand_url")!= null){
                brandUrl.push($(item).attr("brand_url"))
            }
            if ($(item).attr("applet_url") != null){
                appletUrl.push($(item).attr("applet_url"));
            }
            if ($(item).attr("logo_url") != null){
    ...
    layui.use('form', function () {
        ...
        form.on('submit(component-form)', function (data) {
            ...
            if (lnglatlist != null){
                var coordinateList = [];
                var maxLongitude = lnglatlist[0].R;
                var minLongitude = lnglatlist[0].R;
                var maxLatitude = lnglatlist[0].Q;
                var minLatitude = lnglatlist[0].Q;
                //循环找出最大经纬度，最小经纬度
                lnglatlist.forEach(item=>{
                    if (maxLongitude < item.R){
                        maxLongitude = item.R;
                    }
                    if (minLongitude > item.R){
                        minLongitude = item.R;
                    }
                    if (maxLatitude < item.Q){
                        maxLatitude = item.Q;
                    }
                    if (minLatitude > item.Q){
                        minLatitude = item.Q;
                    }
                })
                ...
                var coordinateListStr = JSON.stringify(coordinateList);
                var coordinateListStr1 = $("#coordinateListStr").val()
                searchObj.coordinateChanged = false;
                if (coordinateListStr != coordinateListStr1){
                    searchObj.coordinateChanged = true;
                }
                searchObj.coordinateListStr = coordinateListStr;
                searchObj.serviceNoStr="view_business_update_submission";
                $.simpleAjax('/view/v2/business/view_business_update_submission', 'POST', JSON.stringify(searchObj), "application/json", returnFunction);
    ...
    function onpolygon_creat1(){
        if (lnglatlist1.length = 0||lnglatlist1==null){
            return false;
        }

        var lnglatlistJson =  JSON.parse(lnglatlist1);
        for (var i=0;i<lnglatlistJson.length;i++){
            var tempObj= lnglatlistJson[i];
            var tempArray=[];
            tempArray.push(tempObj.longitude);
            tempArray.push(tempObj.latitude);
            lnglatlist.push(tempArray);
        }
    ...
    function initData() {
        ...
        lnglatlist1 = $("#coordinateListStr").val()
