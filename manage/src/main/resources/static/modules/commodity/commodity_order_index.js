layui.config({
    ...
    table.render({
        ...
        , url: '/sc/manage/commodity/manage_commodity_order_index_data'
        , toolbar: '#order-table-toolbar-toolbarDemo'
        , title: ''
        , cols: [[
            {type: 'checkbox', field: 'id', fixed: 'left'}
            , {field: 'id', title: '订单编号',  width: 200, align: 'center'}
            , {field: 'buyerActualName', title: '买家家名称',  width: 200, align: 'center'}
            , {field: 'commodityName', title: '收货地址',  width: 200, align: 'center'}
            , {field: 'harvestAddress', title: '商品名称',  width: 200, align: 'center'}
    ...
    table.on('tool(order-table-toolbar)', function (obj) {
        ...
        if (obj.event === 'find'){
            ...
            layer.open({
                ...
                area: [width, height],
                title: '订单',
                content: "/sc/manage/commodity/manage_commodity_order_find_page?id="+data.id+"&&commodityId="+data.commodityId
                ,maxmin: true
    ...
    table.on('toolbar(order-table-toolbar)', function(obj){
        ...
        switch(obj.event){
            ...
            case 'cancel_transaction':
                //获取查询表单数据
                d.commodityStatus='cancel_transaction';
                table.reload('order-table-toolbar', {
                    where: d,
                    page: {
                        curr: 1
